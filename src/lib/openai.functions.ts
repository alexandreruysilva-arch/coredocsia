...
/**
 * Extrai valores de indexação de imagens (uma por página) via API OpenAI
 * (chat/completions com visão) e grava log de auditoria em `ai_usage_logs`.
 *
 * FormData: files[] (imagens), fields (JSON), companyId?, documentTypeId?, maxPages?
 */
  ...
  .handler(async ({ data, context }) => {
    ...
    if (orgId) {
      const { data: orgModel } = await supabase
        ...
        .eq("id", orgId)
        .maybeSingle();
      if ((orgModel as { ai_openai_model?: string } | null)?.ai_openai_model) {
        MODEL = (orgModel as { ai_openai_model: string }).ai_openai_model;
      }
    }
    ...
    for (const f of uploadFiles) {
      ...
      if (!/^image\/(jpeg|png|gif|webp)$/.test(mt)) {
        await writeFailureLog({
          ...
          completion: 0,
          total: 0,
          error: `Tipo não suportado pelo OpenAI: ${mt}`,
        });
        throw new Error(
          "OpenAI aceita apenas imagens (JPG, PNG, GIF ou WEBP). Para PDF use Gemini ou Claude.",
        );
      }
    }
...
- Se a informação não for encontrada com confiança, retorne string vazia "".`;
    ...
    // GPT-5.x usa max_completion_tokens e não aceita temperature != 1.
    ...
    if (!resp || !resp.ok) {
      ...
      const friendly =
        lastStatus === 429
          ? "Limite de requisições atingido (OpenAI). Aguarde alguns segundos e tente novamente."
          : lastStatus >= 500
            ? "O serviço de IA (OpenAI) está temporariamente indisponível. Tente novamente."
            : `Falha ao processar via OpenAI (${lastStatus || "rede"}). ${lastErrText.slice(0, 160)}`;
